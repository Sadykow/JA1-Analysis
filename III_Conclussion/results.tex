...
\subsection{Average evaluations and Hyperparameters}
%
% Average 10 attempts with accuracy calculus.
During prelimery work, the research attempted training many models of the same type to a case with a best fitting line.
One of cases was to use a set seed number for all models.
However, determining a case from an infinite number of possibility equaly effective for all models is the least effective method.
Therefore, taking an average results of multiple attempt to smoothen results of history and SoC prediction has been created.
Figure~\ref{fig:Model-DEMO} represents the difference between a single an multiple averaged models, as well as the general spread.
Subfigure~\ref{subfig:Model-DEMO-10} demonstrates how 10 different models, each potentially minimised toward a single minimal, do get spreaded from each other, producing variable results.
...
Due to least trendines plots and clear convergenence or divergence directions, the method proven to be the most objective results.
Nan values were ignored on the cases of longer history***
Similar method has been applied to all SoC plots for all models
%Several evaluation possibilities were tested. In the end, average of 10 provided the most objective results to overview.
%\textcolor{red}{Show plot of 1, then all 10 and average taken.}
\begin{figure*}[htbp]
  \centering
...
Also, it acts as an indication to explore the cell types' limitations and what areas are harder to capture.
...
\textcolor{red}{It is hard to provide any justification, given that average testing history curve only diverges with every epoch.
Due to its' slow fitting process, due to use of inly a single minimal, it could be assumed that majority of the models were focused on a single minimal, without diving too deep to the optimal.
...
\textbf{That is why, the Model \#1 with a general optimiser probably can be considred as the best genral use combination, since it does not go too far into achiving lowest minimum and not experiancing overfit with other models.}
