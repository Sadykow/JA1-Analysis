...
However, it did not receive wide application due to the battery's initial state's requirement and prior utilisation as input requirements.
...
%
%
\textcolor{red}{Both stateful and stateless methods fall for input samples' impact and their size on model estimation accuracy, which became the research source [ref to some comparisons].
...
Mamo et al.~\cite{mamo_long_2020} conducted experiments by validating the RNN model performance by training one \textcolor{red}{driving profile} and testing against another.
