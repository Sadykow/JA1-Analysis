...
The research conducted by~\cite{mamo_long_2020} was intended to determine weaknesses and improve the model introducing addition techniques into the default structure of training model.
He added Attention Layer~\cite{yang_hierarchical_2016} in addition between LSTM and fully connected layers to improve accuracy, along with replacing traditional gradient optimiser with probability based Differential Evolution.
...
The implementation of the Attention layer does not get provided with Machine Learning library, the source code from Winata research~\cite{winata_attention-based_2018} has been used instead.
The open source code publickly accesses through Github source~\cite{attention_8461990}.
Details in terms of optimiser usage and replacement justefied in the subsection~\ref{subsec:optimisers}.
