...
In addition to a second $\beta$ constant at~\ref{alg:Adam-Line-2Moment} used for momentum calculation, the algorithm uses $\epsilon$, referred to as the fuzz factor.
...
%
Javid et al.~\cite{javid_adaptive_2020} extended the default $Adam$ algorithm introducing a Robust Online version of Adam \mbox{Algorithm~\ref{alg:RoAdam}}, Line~\ref{alg:RoAdam-Line-Online}, along with third $\beta$ constant at Line~\ref{alg:RoAdam-Line-3Moment}.
...
\begin{algorithm}\captionsetup{labelfont={sc,bf}, labelsep=newline}
  ...
  \begin{algorithmic}[1]
    ...
    \STATE $d_t \gets \beta_3 d_{t-1}+\left(1-\beta_3\right)r_t $ \COMMENT{$3^{rd}$ moment calculation \label{alg:RoAdam-Line-3Moment}}
...
{}
Unlike initial variables, $m$, and $v$ of the Adam algorithm, which get set as matrices of zeros, the $d$ variable in the RoAdam is initialised with ones, Line~\ref{alg:RoAdam-Line-Vars}.
...
The initial loss value must be set above zero per the algorithm to avoid a zero-division error, Line~\ref{alg:RoAdam-Line-Loss}.
% Optimiser implementation from the first principle remains the most viable option for the current version of TensorFlow, there as PyTorch already has the variation inbuilt to the standard library.
