...
One of the simplest methods to optimise the model is the Stochastic Gradient Descent (SGD), Algorithm~\ref{alg:SGDwM}. %, \mbox{Algorithm~\ref{alg:SGD}}.
...
\begin{algorithm}\captionsetup{labelfont={sc,bf}, labelsep=newline}
  ...
  \begin{algorithmic}[1]
    ...
    \WHILE{$W_t \text{ not converge}$}
    \STATE $t \gets t+1$
    \STATE $g_t \gets \nabla_\phi L_t (W_{t-1})$ \COMMENT{Obtain gradient \label{alg:SGDwM-Line-Gradient}}
